...
import re
import numpy as np
data_dir = 'D:/项目/咕泡项目/自然语言处理NLP/命名提识别/ner/data1'
...
###是否预先表计划is_split_into_words；return_offsets_mapping立原始文本中的字符位置和分词后标记（tokens）之间的映射关系
...
model = AutoModelForTokenClassification.from_pretrained('CIRCL/vulnerability-severity-classification-distilbert-base-uncased',num_labels=2,
...
training_args = TrainingArguments(
    output_dir='C:\\PerfLogs\\NLP\\logs',          # 输入路径
    ...
    logging_dir='C:\\PerfLogs\\NLP\\\output',
    logging_steps=10,
    save_strategy='steps',
    save_steps=1000,
    save_total_limit=1,
...
model.save_pretrained("./checkpoint/model/class_%s-%sepoch" % (checkpoint, num_train_epochs))